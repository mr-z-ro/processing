...
class Crack {
  ...
  void move() {
    ...
    if ((cx>=0) && (cx<width) && (cy>=0) && (cy<height)) {
      // safe to check
      if ((cgrid[cy*width+cx]>10000) || (abs(cgrid[cy*width+cx]-t)<5)) {
        // continue cracking
        cgrid[cy*width+cx]=int(t);
      } else if (abs(cgrid[cy*width+cx]-t)>2) {
